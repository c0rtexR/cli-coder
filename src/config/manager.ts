  ...
  private applyEnvironmentOverrides(config: Partial<AppConfig>): void {
    ...
    if (process.env.OPENAI_API_KEY) {
      config.llm = { 
        ...
        provider: (config.llm?.provider || 'openai') as 'openai' | 'anthropic' | 'gemini',
    ...
    if (process.env.ANTHROPIC_API_KEY) {
      config.llm = { 
        ...
        provider: (config.llm?.provider || 'anthropic') as 'openai' | 'anthropic' | 'gemini',
    ...
    if (process.env.CLI_CODER_SHELL_TIMEOUT) {
      ...
      if (!isNaN(timeout) && config.shell) {
        config.shell = { ...config.shell, defaultTimeout: timeout };
      }
    }
    
    if (process.env.CLI_CODER_ALLOW_DANGEROUS === 'true' && config.shell) {
      config.shell = { ...config.shell, allowDangerousCommands: true };
    }
  }
}